...
import { IpfsFile, IpfsFile1, IpfsFile3, IpfsFile4 } from "../generated/schema";
...
export function handleTestEvent(event: TestEvent): void {
  ...
  } else if (command == "createFile3") {
    // Will fail the subgraph with a conflict between two entities created by offchain data sources.
    let context = new DataSourceContext();
    context.setBytes("hash", event.block.hash);
    dataSource.createWithContext("File3", [KNOWN_CID], context);
  } else {
...
export function handleFile3(data: Bytes): void {

  let context = new DataSourceContext();
  context.setBytes("file", Bytes.fromUTF8("fromHandleFile3"));
  dataSource.createWithContext("File4", [KNOWN_CID], context);
}

export function handleFile4(data: Bytes): void {
  let entity = new IpfsFile4(dataSource.stringParam());
