...
async fn file_data_sources() {
    ...
    let blocks = {
        ...
        push_test_log(&mut block_5, "createFile3");
    ...
    let query_res = ctx
        .query(&format!(
            r#"{{ ipfsFile4(id: "{id}") {{ id, content, context }} }}"#,
        ))
        .await
        .unwrap();
    ...
    assert_json_eq!(
        query_res,
        Some(
            object! { ipfsFile4: object!{ id: id , content: content.clone(), context: "fromHandleFile32" } }
        )
    );
    ...
    let message = "entity type `IpfsFile1` is not on the 'entities' list for data source `File2`. \
                   Hint: Add `IpfsFile1` to the 'entities' list, which currently is: `IpfsFile`.\twasm backtrace:\t    0: 0x36d6 - <unknown>!src/mapping/handleFile1\t in handler `handleFile1` at block #7 ()".to_string();
